 ...
 *
 * Function to compute the Gaussian pdf (probability density function) and the Gaussian cdf
 ...
 *
 * The approximation is accurate to absolute error less than 8 * 10^(-16). Reference: Evaluating the
 * Normal Distribution by George Marsaglia. http://www.jstatsoft.org/v11/a04/paper
 *
 ******************************************************************************/
...
public class Gaussian {
  ...
  // Mean of a truncated normal law.
  // Values of this variable are in the range [a,b]
  public static double truncatedMean(double mu, double sigma, double a, double b) {
    double alpha = (a - mu) / sigma;
    double beta = (b - mu) / sigma;
    if (alpha >= 8 && beta >= 8)
      return 0;
    else if (alpha <= -8 && beta >= 8)
      return mu;
    else if (alpha <= -8 && beta <= -8)
      return 0;
    return mu - sigma * (Gaussian.pdf(beta) - Gaussian.pdf(alpha))
        / (Gaussian.cdf(beta) - Gaussian.cdf(alpha));
  }

  // Mean of a truncated normal law.
  // Values of this variable are in the range [a,b]
  public static double leftTruncatedMean(double mu, double sigma, double a) {
    double alpha = (a - mu) / sigma;
    if (alpha >= 8)
      return 0;
    else if (alpha <= -8)
      return 0;
    double cdf1 = Gaussian.cdf(alpha);
    double pdf1 = Gaussian.pdf(alpha);
    return mu + sigma * pdf1 / (1 - cdf1);
  }
